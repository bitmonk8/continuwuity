...
mod platform {
	...
	/// Stub: device parallelism detection is only available on Linux via sysfs.
	#[must_use]
	pub fn parallelism(_path: &Path) -> Parallelism { Parallelism::default() }

	/// Stub: block device name detection is only available on Linux via sysfs.
	pub fn name_from_path(_path: &Path) -> crate::Result<String> {
...
pub use platform::{name_from_path, parallelism};
